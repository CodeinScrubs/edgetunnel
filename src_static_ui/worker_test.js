			...
			if (env.KV && typeof env.KV.get === 'function') {
				...
				if (区分大小写访问路径 === 加密秘钥 && 加密秘钥 !== 'default-key-change-with-KEY-env-if-needed') {
					...
					if (访问路径 === 'admin/log.json') {
						...
						try {
							const 凭据 = request.method === 'POST' ? await request.json().catch(() => ({})) : {}; const Usage_JSON = await getCloudflareUsage(凭据.Email, 凭据.GlobalAPIKey, 凭据.AccountID, 凭据.APIToken);
					...
					if (访问路径 === 'admin/init') {
						...
						let 重置确认 = null;
						try { 重置确认 = (await request.json())?.confirm } catch (e) { }
