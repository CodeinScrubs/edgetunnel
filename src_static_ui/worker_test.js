...
const Version = '2026-07-31 src:ad9c44cbbf42 panel';
...
async function 处理WS请求(request, yourUUID, url) {
	...
	const 处理WS入站数据 = async (chunk) => {
		...
		// message boundary is not a protocol boundary: a client is free to split a valid VLESS/Trojan header
			...
			return {
				effective: 结果.status === 'valid' ? 'explicit UUID accepted'
					: 结果.status === 'absent' ? 'derived from ADMIN/KEY'
						: `explicit UUID REJECTED (${结果.reason}) - clients using it will fail to authenticate`,
