...
| **`FIRST_BYTE_TIMEOUT_MS`** | `0` (off) | 0–10000 | If a connection *opens* but sends **no data** within this many ms, cancel it. On the **direct** route it then falls back to the ProxyIP relay; on the **ProxyIP** route it closes so the client re-dials. | **Off by default.** Turn on (try `1500`–`2500`) **only if you get "connected but page loads forever"** — it rescues blackholed routes on both direct and ProxyIP paths. **Cons:** a genuinely slow server (>your value to first byte) gets closed; and on the ProxyIP path a closed connection makes the client re-dial, which re-sends a non-idempotent first packet (e.g. a POST) if the origin had received it but was just slow — so set it generously. |
